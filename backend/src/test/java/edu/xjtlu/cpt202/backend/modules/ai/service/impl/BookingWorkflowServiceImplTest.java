...
class BookingWorkflowServiceImplTest {
    ...
    @Test
    void shouldReturnAllDaySlotsWhenUserProvidesDateOnly() {
        InMemoryBookingTaskStateStore store = new InMemoryBookingTaskStateStore();
        SpecialistQueryService specialistQueryService = mock(SpecialistQueryService.class);
        BookingWorkflowServiceImpl service = new BookingWorkflowServiceImpl(
                store,
        ...
        SpecialistDetailVO detailTwo = new SpecialistDetailVO();
        detailTwo.setId(102L);
        detailTwo.setName("Amy");
        detailTwo.setConsultationFee(new BigDecimal("180.00"));
        detailTwo.setStatus("ACTIVE");
        when(specialistQueryService.getSpecialistDetail(102L)).thenReturn(detailTwo);

        when(specialistQueryService.listAvailability(101L, date)).thenReturn(List.of(
                slot(9901L, date, "03:30", "04:00"),
                slot(9902L, date, "11:00", "11:30")
        ));
        when(specialistQueryService.listAvailability(102L, date)).thenReturn(List.of(
                slot(9903L, date, "19:30", "20:00")
        ));

        String reply = service.handle(1001L, wrapped("\u5e2e\u6211\u770b5\u67086\u53f7\u4e13\u5bb6\u53ef\u9884\u7ea6\u7684"));

        assertThat(reply).contains("| 101 | Lic | 2026-05-06 | 9901 | 03:30 | 04:00 | 200.00 |");
        assertThat(reply).contains("| 101 | Lic | 2026-05-06 | 9902 | 11:00 | 11:30 | 200.00 |");
        assertThat(reply).contains("| 102 | Amy | 2026-05-06 | 9903 | 19:30 | 20:00 | 180.00 |");
        assertThat(reply).doesNotContain("\u8bf7\u544a\u8bc9\u6211\u65f6\u6bb5");
        assertThat(store.get(1001L)).isPresent();
        assertThat(store.get(1001L).get().getCandidateSlots()).hasSize(3);
    }

    @Test
    void shouldFallbackToAnyTimeSlotsWhenRequestedPeriodHasNoMatches() {
        InMemoryBookingTaskStateStore store = new InMemoryBookingTaskStateStore();
        SpecialistQueryService specialistQueryService = mock(SpecialistQueryService.class);
        BookingWorkflowServiceImpl service = new BookingWorkflowServiceImpl(
                store,
        ...
        LocalDate date = LocalDate.parse("2026-05-07");
        ...
        when(specialistQueryService.listAvailability(101L, date)).thenReturn(List.of(
                slot(9501L, date, "09:00", "09:30")
        ));

        String reply = service.handle(1001L, wrapped("\u6211\u8981\u9884\u8ba25\u67087\u53f7\u7684\u4e13\u5bb6\uff0c\u4e0b\u5348\u65f6\u95f4\u6bb5"));

        assertThat(reply).contains("\u6ca1\u6709\u4e0b\u5348\u65f6\u6bb5");
        assertThat(reply).contains("\u8fd9\u5929\u4e0a\u5348\uff08\u65e9\u4e0a\uff09\u6709\u7a7a\u4f59");
        assertThat(reply).contains("| 101 | Lic | 2026-05-07 | 9501 | 09:00 | 09:30 | 200.00 |");
        assertThat(store.get(1001L)).isPresent();
        assertThat(store.get(1001L).get().getCandidateSlots()).hasSize(1);
    }
    ...
    @Test
    void shouldFallbackToOtherPeriodsWhenEveningHasNoMatches() {
        InMemoryBookingTaskStateStore store = new InMemoryBookingTaskStateStore();
        SpecialistQueryService specialistQueryService = mock(SpecialistQueryService.class);
        BookingWorkflowServiceImpl service = new BookingWorkflowServiceImpl(
                store,
        ...
        when(specialistQueryService.getSpecialistDetail(101L)).thenReturn(detail);
        when(specialistQueryService.listAvailability(101L, date)).thenReturn(List.of(
                slot(9801L, date, "11:53", "12:23")
        ));

        String reply = service.handle(1001L, wrapped("\u5e2e\u6211\u770b5\u67085\u53f7\u665a\u4e0a\u6709\u65f6\u95f4\u7684\u4e13\u5bb6\uff0c\u6211\u8981\u9884\u8ba2"));

        assertThat(reply).contains("\u6ca1\u6709\u665a\u4e0a\u65f6\u6bb5");
        assertThat(reply).contains("\u8fd9\u5929\u4e0a\u5348\uff08\u65e9\u4e0a\uff09\u6709\u7a7a\u4f59");
        assertThat(reply).contains("| 101 | Lic | 2026-05-05 | 9801 | 11:53 | 12:23 | 200.00 |");
    }

    @Test
    void shouldNotTreatMorningAsSpecialistNameWhenUserAsksDateThenExperts() {
        InMemoryBookingTaskStateStore store = new InMemoryBookingTaskStateStore();
        SpecialistQueryService specialistQueryService = mock(SpecialistQueryService.class);
        BookingWorkflowServiceImpl service = new BookingWorkflowServiceImpl(
                store,
        ...
        when(specialistQueryService.getSpecialistDetail(101L)).thenReturn(detail);
        when(specialistQueryService.listAvailability(101L, date)).thenReturn(List.of(
                slot(9601L, date, "09:30", "10:00")
        ));

        String reply = service.handle(1001L, wrapped("\u5e2e\u6211\u770b5\u67086\u53f7\u6709\u65f6\u95f4\u7684\u4e13\u5bb6\u65e9\u4e0a\uff0c\u6211\u8981\u9884\u8ba2"));

        assertThat(reply).contains("| Specialist ID | Specialist | Date | Slot ID | Start | End | Fee |");
        assertThat(reply).contains("| 101 | Lic | 2026-05-06 | 9601 | 09:30 | 10:00 | 200.00 |");
        assertThat(reply).doesNotContain("I could not find available slots for that specialist/date");
    }
